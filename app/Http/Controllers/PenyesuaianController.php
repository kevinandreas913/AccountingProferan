...
class PenyesuaianController extends BaseController
{
    ...
    {
        return view('pages.penyesuaian.index');
    }

    public function table(Request $request)
    {
        $data = Transaksi::where('user_id', Auth::user()->id)->where('type', 'penyesuaian')->where(function ($q) use ($request) {
    ...
    public function createUpdate(Request $request)
    {
        ...
        if (empty($request->id)) {
            $result = $this->moduleService->processModuleCreate('penyesuaian', $request);
            $response = $result->getData();
        } else {
            $result = $this->moduleService->processModuleUpdate('penyesuaian', $request);
            $response = $result->getData();
        }
    ...
    {
        try {
            $transaksi = Transaksi::where('id', $id)->where('user_id', Auth::user()->id)->where('type', 'penyesuaian')->firstOrFail();
            return $this->sendResponse($transaksi, 'Data ditemukan', 'success', 200);
        } catch (\Exception $e) {
    ...
    {
        $result = $this->moduleService->processModuleDestroy('penyesuaian', $request);
        $response = $result->getData();
