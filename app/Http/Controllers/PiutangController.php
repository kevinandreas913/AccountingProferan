...
class PiutangController extends BaseController
{
    ...
    {
        $data = Transaksi::where('user_id', Auth::user()->id)->where('type', 'piutang')->where(function ($q) use ($request) {
    ...
    public function createUpdate(Request $request)
    {
        ...
        if (empty($request->id)) {
            $result = $this->moduleService->processModuleCreate('piutang', $request);
            $response = $result->getData();
        } else {
            $result = $this->moduleService->processModuleUpdate('piutang', $request);
            $response = $result->getData();
        }
    ...
    {
        try {
            $transaksi = Transaksi::where('id', $id)->where('user_id', Auth::user()->id)->where('type', 'piutang')->firstOrFail();
    ...
    {
        $result = $this->moduleService->processModuleDestroy('piutang', $request);
        $response = $result->getData();
