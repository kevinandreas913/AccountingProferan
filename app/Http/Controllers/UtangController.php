...
class UtangController extends BaseController
{
    ...
    {
        $data = Transaksi::where('user_id', Auth::user()->id)->where('type', 'utang')->where(function ($q) use ($request) {
    ...
    public function createUpdate(Request $request)
    {
        ...
        if (empty($request->id)) {
            $result = $this->moduleService->processModuleCreate('utang', $request);
            $response = $result->getData();
        } else {
            $result = $this->moduleService->processModuleUpdate('utang', $request);
            $response = $result->getData();
        }
    ...
    {
        try {
            $transaksi = Transaksi::where('id', $id)->where('user_id', Auth::user()->id)->where('type', 'utang')->firstOrFail();
    ...
    {
        $result = $this->moduleService->processModuleDestroy('utang', $request);
        $response = $result->getData();
